...
public class GitRepoFactory {
    ...
    public static GitRepository createSSHGitRepository(TextProvider txtProvider) {
	...
	public static GitRepository createSSHGitRepository(String url, String username, 
			String password, String branch, File sshKeyFile, String sshPassphrase, 
			boolean useShallowClones, boolean useSubmodules, int timeout, 
			boolean verbose, TextProvider txtProvider) {
		GitRepository gitRepository = new GitRepository();
		
		configureSSHGitRepository(gitRepository, url, username, password, branch,
				sshKeyFile, sshPassphrase, useShallowClones, useSubmodules,
				timeout, verbose, txtProvider);
        
        return gitRepository;
	}
	
	public static void configureSSHGitRepository(GitRepository g, String url, String username, 
			String password, String branch, File sshKeyFile, String sshPassphrase, 
			boolean useShallowClones, boolean useSubmodules, int timeout, 
			boolean verbose) {
        ...
        buildConfig.setProperty(REPOSITORY_GIT_AUTHENTICATION_TYPE, GitAuthenticationType.SSH_KEYPAIR.name());
        buildConfig.setProperty(REPOSITORY_GIT_USE_SHALLOW_CLONES, new Boolean(useShallowClones));
        buildConfig.setProperty(REPOSITORY_GIT_USE_SUBMODULES, new Boolean(useSubmodules));
        buildConfig.setProperty(REPOSITORY_GIT_VERBOSE_LOGS, new Boolean(verbose));
        
        buildConfig.setProperty(REPOSITORY_GIT_COMMAND_TIMEOUT, DEFAULT_COMMAND_TIMEOUT_IN_MINUTES);
        buildConfig.setProperty(TEMPORARY_GIT_PASSWORD, password);
        buildConfig.setProperty(TEMPORARY_GIT_SSH_PASSPHRASE_CHANGE, new Boolean(true));
        buildConfig.setProperty(TEMPORARY_GIT_SSH_PASSPHRASE,sshPassphrase);
        buildConfig.setProperty(TEMPORARY_GIT_SSH_KEY_CHANGE, new Boolean(true));
